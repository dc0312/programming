package recursion;

import java.util.ArrayList;
import java.util.List;

/*
*Given a string, we have to find out all subsequences of it. A String is a subsequence of a given String, that is generated by deleting some character of a given string without changing its order.
Examples:

Input : abc
Output : a, b, c, ab, bc, ac, abc

Input : aaa
Output : a, aa, aaa
* */
public class PrintAllSubsequences {
    public static void main(String[] args) {
        String s = "aba";
        printAllSubsequences(s);
    }

    private static void printAllSubsequences(String s) {
        String op = "";
        List<String> ans = new ArrayList<>();
        printAllSubsequences(s, op, ans);
        ans.stream().forEach(System.out::println);
        System.out.println(ans.stream().filter(x -> x.length() > 0).count());
    }

    private static void printAllSubsequences(String s, String op, List<String> ans) {
        if (s.length() == 0) {
            ans.add(op);
            return;
        }
        printAllSubsequences(s.substring(1), op, ans);
        printAllSubsequences(s.substring(1), op + s.charAt(0), ans);
    }
}
